...
-   Challenge: Modeling maximum daily precipitation in yearly blocks
    ...
    -   Location-specitiv GEVt distributions
...
- **GEV model for precipitation extremes**  
  ...
  3) *Hyper-parameters:* $\theta =$ f.ex. mean, range, variance, spatial correlation
...
- **Joint GEV model for precipitation extremes**  
  ...
  2) *Latent field:* $\boldsymbol\eta_i = \begin{pmatrix} (\psi_1, \tau_1, \phi_1)^T \\  \vdots \\ (\psi_n, \tau_n, \phi_n)^T \end{pmatrix}  \sim \mathcal{N}(\boldsymbol{\mu}(\theta), \mathbf{Q}^{-1}(\theta))$ 
...
:::
:::
1. Get MLEs and Hessians from TMB (7s on 70x91 UKCP over Island of Ireland)
2. Pass $\hat \eta$ and CSR version of $L_{\eta y}$ into Stan (39s)
3. Stan gives full posterior of Gaussian-Gaussian model (10m)
:::
