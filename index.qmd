...
- **Two stages** 
  1. **Max:** approximate each replicate-specific likelihood by a Gaussian.  
  2. **Smooth:** fuse those Gaussian pseudo-observations with the latent GMRF prior.
- Preserves full-rank uncertainty while scaling to $10^5+$ latent variables.
:::

## Max step — Gaussian likelihood surrogate

::: incremental
- Find MLE $\hat\eta$ and observed information $I_{\eta y}$.  
  $$\hat L(\eta\mid y)=\mathcal N\!\bigl(\eta\mid\hat\eta,\;I_{\eta y}^{-1}\bigr)$$
- Block-diagonal across independent replicates → embarrassingly parallel.
:::
...
- Combines all information in closed-form; sampling or Laplace/INLA possible.
